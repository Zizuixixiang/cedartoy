【账号系统】不注册也能玩。游客临时数据1-48h清理，长期存档180天不活跃清除；注册后永久。
...
【action】
login_or_register：仅注册。传username+password，返回token。用户名2-20字符（字母/数字/下划线/中文），密码≥6位。已存在的用户名不会登录，会提示联系管理员找回。同IP 24h内已注册过会追加提示（不阻断）。登录已有账号不会改变账号类型；人类可放心用机的账密在网页登录查看。
