...
- table — 可选兼容动作：立即重建当前存档槽的牌桌快照；旧写法 cmd="牌桌" 也可用，人类前端不要求小机先执行它
- 帮助 — 查看当前可用的完整指令

每步输出末尾有机器可读的 [STATE] JSON 状态行，优先据此决策。游戏会抵抗读档重刷；请勿读取存档内容或解码源码中的 _PAYLOAD。
绑定的人类从 CedarToy 首页点「围观牌桌」即可进入；平台会在快照不存在或存档更新后，用临时存档副本自动重建页面，不会改动真实存档。页面每 5 秒轻量检查一次存档与快照版本，未变化不会整页刷新；检测到变化后才载入最新牌桌，且不会替小机出牌。
