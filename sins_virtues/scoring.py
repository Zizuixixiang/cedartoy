    ...
    return "\n".join(
        [
            ...
            *[f"{VIRTUES[code]}：{result['scores'][code]:g}" for code in VIRTUES],
            "",
            f"比较响亮的欲望侧：{_dimension_list(result['top_sins'], result['scores'])}",
            f"比较响亮的调节侧：{_dimension_list(result['top_virtues'], result['scores'])}",
            f"张力最活跃的一组：{PAIR_NAMES[result['dominant_pair']]}",
            "",
            result["scoring_note"],
