...
- `POST /ask`：提问。body: `{room_id, content}`。
  ...
  - 调用 `judge_ask(surface, answer, question)`：模型首行须为「是 / 不是 / 无关 / 是也不是」，映射为 `yes/no/unrelated/partial`；裁判失败时写入 `content_override` 提示重试并记为 `unrelated`。
  - 若模型回复含 `【线索公布】` 行，额外写入 `auto_hint` 并广播 `new_log`（前端横幅标题「【线索公布】」）。
...
- `POST /guess`：猜汤底。body: `{room_id, content}`。
  - 调用 `judge_guess(surface, answer, guess)`，解析 `【通关】/【未通关】` 与还原度百分比；日志 `content` 含「还原度：N%」。
  - 猜中时：房间 `finished`，写 `winner_id/finished_at`，胜者 `win_count+1`，所有有 ask/guess 记录的玩家 `game_count+1`，揭晓日志优先使用模型 `【汤底】` 段，广播 `new_log` 与 SSE `game_over`。
...
系统 prompt 来源：优先 `settings` 表中的 `judge_prompt` / `generate_prompt`（非空）；否则回退 `config/judge_prompt.txt` 或 `DEFAULT_SETTINGS`。

裁判函数：

- `judge_ask(surface, answer, question) -> {judgment, clue?, content_override?}`
  - 使用 `judge_prompt`；首行合法值为「是 / 不是 / 无关 / 是也不是」；`【线索公布】` 行解析为 `clue`。
  - `_chat_validated` 最多重试 3 次；仍失败则 `unrelated` + `content_override`。
- `judge_guess(surface, answer, guess) -> {success, score, answer?, error?}`
  - 解析 `【通关】/【未通关】`、还原度与可选 `【汤底】`；失败时 `error` 提示重试。
- `generate_hint(surface, answer, game_log) -> str | None`
  - 使用最近最多 40 条 ask 记录；回复须以 `【提示】` 开头，返回最多 120 字；失败返回 `None`。
- `generate_puzzle() -> {surface, answer}`
  - 使用 `generate_prompt`；要求模型返回 JSON。
  - 解析失败返回 HTTP 502。
...
| 路由 | 页面 | 说明 |
| --- | --- | --- |
| `/soup/` | `Lobby` | 像素风大厅：房间列表（含 48h 内已结束房）、汤名搜索与标签筛选、创建房间防重复提交 |
| `/soup/room/:id` | `Room` | 对局页：SSE 订阅、`presence.enter_room`；判定前缀中文展示；`auto_hint`→「【线索公布】」、`hint_offer`→「【请求提示】」（接受/拒绝，上限 3 次，请求前确认）；记事本侧栏/抽屉 |
| `/soup/profile` | `Profile` | 登录用户资料 |
| `/soup/admin` | `Admin` | 管理后台（需 `is_admin`）；裁判 API 弹窗支持「测试」「拉取模型」；settings `value` 用 `textarea` |
