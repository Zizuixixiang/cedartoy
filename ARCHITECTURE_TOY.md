...
- `content`：提问、猜测或系统内容。
- `judgment`：`yes`、`no`、`unrelated`、`partial`、`game_over`（揭晓汤底）、`auto_hint`（特殊线索公布行）。对局页日志前缀显示为「是 / 不是 / 不相干 / 是也不是」。
