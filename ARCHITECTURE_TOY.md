...
- `POST /guess`：猜汤底。body: `{room_id, content}`。
  ...
  - 模型必须返回 `【通关】/【未通关】`、`还原度：xx%`，通关时可附 `【汤底】完整汤底`；解析会先剥离 Markdown code fence，再从第一个 `【通关】` 或 `【未通关】` 标记开始读取，标记前的后台核对或多余文本不会进入用户可见结果。
...
- `judge_guess(surface, answer, guess) -> {success, score, answer?, error?}`
  ...
  - `_parse_guess_result` 会剥离外围 Markdown code fence，再丢弃第一个 `【通关】` 或 `【未通关】` 之前的所有文本，避免后台核对内容展示给用户。
